"""
═══════════════════════════════════════════════════════════
  微台期｜開盤15分K突破策略【v5.3 最佳化版】
═══════════════════════════════════════════════════════════
  策略規則：
    1. 只做 ATR ≥ 歷史第25百分位的日子（排除盤整日）
    2. 只接受 09:15 之後觸發的突破（排除開盤假突破）
    3. 突破棒量能 ≥ 前5根均量 × 1.5（排除低量假突破）
    4. RSI(14) 動能確認：做多 RSI>50，做空 RSI<50
    5. 突破方向需與當日 VWAP 一致（多頭在均價上方、空頭在下方）
    6. 嚴格 ATR×0.4 停損，不移動
    7. 移動停利：獲利30點觸發，回撤20點出場
    8. 固定停利：ATR×1.5
    9. 每日只做第一個訊號，尾盤強制平倉
═══════════════════════════════════════════════════════════
"""
...
TRAIL_DISTANCE  = 20
ATR_FILTER_PCT  = 25
VOL_MULT        = 1.5   # 突破棒量 >= 前N根均量 × 倍數
VOL_LOOKBACK    = 5     # 回看前幾根15分K的均量
...
# ══ 回測 ══
def run_backtest(df15, atr_map, vwap_idx, date_start=None, date_end=None):
    ...
    for trade_date, group in df15.groupby('Date'):
        ...
        for _, bar in subsequent.iterrows():
            ...
            if direction is None:
                ...
                if cand:
                    # 量能確認：突破棒量 >= 前 VOL_LOOKBACK 根均量 × VOL_MULT
                    pos_list = df15.index[df15['DateTime'] == bar['DateTime']]
                    if len(pos_list) == 0:
                        continue
                    pos = pos_list[0]
                    if pos < VOL_LOOKBACK:
...
def print_report(t):
    ...
    mo     = max(days/30,1)
    # 點數版（不含手續費，純損益點）
    pts_eq  = t['損益點數'].cumsum()
    mdd_pts = (pts_eq-pts_eq.cummax()).min()
    net_pts = t['損益點數'].sum()

    print('\n'+'='*68)
    print('  微台期｜開盤15分K突破策略 v5.3（最佳化版）')
    print('='*68)
    print(f'  回測期間  ：{t["日期"].min()} ~ {t["日期"].max()}')
    print(f'  進場條件  ：ATR≥P25 + 09:15 + 量能×1.5 + RSI>50/<50 + VWAP')
    ...
    print(f'  獲利因子 ：{pf:.2f}')
    print(f'  最大回撤 ：{mdd_pts:,.1f} 點  （微台 {mdd:,.0f}元 / 小台 {mdd_pts*50:,.0f}元）')
    print(f'  累積淨利 ：{net_pts:,.0f} 點  （微台 {net:,.0f}元 / 小台 {net_pts*50:,.0f}元）')
    print(f'  月均損益 ：{net_pts/mo:,.1f} 點  （微台 {net/mo:,.0f}元 / 小台 {net_pts/mo*50:,.0f}元）')
    print('-'*62)
    print('  出場原因分佈：')
    ...
    # 逐年統計
    print('\n  ── 逐年損益（驗證穩定性）──')
    print(f"  {'年份':>5}  {'次數':>5}  {'勝率':>7}  {'PF':>6}  {'淨利(點)':>10}  {'MDD(點)':>9}  {'小台淨利':>10}")
    print('  ' + '-'*66)
    ...
    for yr, g in t.groupby('年份'):
        ...
        print(f'  {yr}  {len(g):>5}  {y_wr:>6.1f}%  {y_pf:>5.2f}  {y_pts:>+9.0f}點  {y_mdd:>8.0f}點  {y_pts*50:>+9,.0f}元')
...
def plot_results(t):
    ...
    plt.tight_layout()
    plt.savefig('backtest_v53_result.png', dpi=150, bbox_inches='tight', facecolor='#0f172a')
    print('\n圖表已儲存：backtest_v53_result.png')

if __name__ == '__main__':
    print('微台期 開盤15分K突破策略 v5.3 啟動')
    print('進場條件：ATR≥P25 + 09:15 + 量能×1.5 + RSI>50/<50 + VWAP\n')
    ...
    print_report(trades)
    trades.to_csv('trades_v53_detail.csv', index=False, encoding='utf-8-sig')
    print('交易明細已儲存：trades_v53_detail.csv')
    plot_results(trades)

    # 樣本外單獨報告（2017～2023）
    print('\n' + '='*62)
    print('  ★ 樣本外驗證：2017～2023（從未參與優化的資料）')
    print('='*62)
    oos = run_backtest(df15, atr_map, vwap_idx,
                       date_start='2017-01-01', date_end='2023-12-31')
    print_report(oos)
    print(f"\n  {'年份':>5}  {'次數':>5}  {'勝率':>7}  {'PF':>6}  {'盈虧比':>7}  {'月均':>9}  {'年淨利':>10}  {'MDD':>9}")
    print('  ' + '-'*68)
    oos['年份'] = pd.to_datetime(oos['日期']).dt.year
    for yr, g in oos.groupby('年份'):
        gw = g[g['損益點數']>0]; gl = g[g['損益點數']<0]
        yp  = gw['損益點數'].sum()/abs(gl['損益點數'].sum()) if len(gl) else 99
        yrr = abs(gw['損益點數'].mean()/gl['損益點數'].mean()) if len(gl) else 99
        g2  = g.copy()
        g2['月份'] = pd.to_datetime(g2['日期']).dt.to_period('M')
        ym  = g2['淨損益(元)'].sum() / len(g2.groupby('月份'))
        yeq = g2['淨損益(元)'].cumsum()
        ymdd= (yeq - yeq.cummax()).min()
        print(f"  {yr}  {len(g):>5}  {len(gw)/len(g)*100:>6.1f}%  {yp:>5.2f}  {yrr:>6.2f}:1  {ym:>8,.0f}元  {g['淨損益(元)'].sum():>9,.0f}元  {ymdd:>8,.0f}元")
