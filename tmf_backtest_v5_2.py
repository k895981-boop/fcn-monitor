...
def load_and_prepare(filepath):
    ...
    df15 = df15.sort_values('DateTime').reset_index(drop=True)  # 全局索引供量能查詢
...
def print_report(t):
    ...
    print('\n'+'='*62)
    print('  微台期｜開盤15分K突破策略 v5.2（最佳化版）')
    print('='*62)
    print(f'  回測期間  ：{t["日期"].min()} ~ {t["日期"].max()}')
    print(f'  進場條件  ：ATR≥P25 + 09:15後進場 + 量能×1.5 + VWAP方向一致')
...
def plot_results(t):
    ...
    plt.tight_layout()
    plt.savefig('backtest_v52_result.png', dpi=150, bbox_inches='tight', facecolor='#0f172a')
    print('\n圖表已儲存：backtest_v52_result.png')

if __name__ == '__main__':
    print('微台期 開盤15分K突破策略 v5.2 啟動')
    print('進場條件：ATR≥P25 + 09:15後進場 + 量能×1.5 + VWAP方向確認\n')
    ...
    print_report(trades)
    trades.to_csv('trades_v52_detail.csv', index=False, encoding='utf-8-sig')
    print('交易明細已儲存：trades_v52_detail.csv')
    plot_results(trades)
